...
bool PearlPID::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
	...
	if(MOOSStrCmp(msg.m_sVal, "FALSE")) {
	  m_has_control = true;
	  MOOSTrace("\n");
	  MOOSDebugWrite("pPearlPID Control Is On");
	}
	else if(MOOSStrCmp(msg.m_sVal, "TRUE")) {
	  if(m_allow_overide) {
	    m_has_control = false;
	    MOOSTrace("\n");
	    MOOSDebugWrite("pPearlPID Control Is Off");
	  }
	}
      }
...
bool PearlPID::OnConnectToServer()
{
  registerVariables();
  return(true);
}
...
void PearlPID::registerVariables()
{
...
bool PearlPID::OnStartUp()
{
  AppCastingMOOSApp::OnStartUp();
  cout << "pPearlPID starting...." << endl;
...
bool PearlPID::handleYawSettings()
{
  int ok = true;
...
bool PearlPID::handleSpeedSettings()
{
  int ok = true;
  m_pengine.setSpeedFactor(m_speed_factor);
...
bool PearlPID::handleDepthSettings()
{
  int ok = true;
...
double PearlPID::angle360(double degval)
{
  while(degval >= 360.0)
    degval -= 360.0;
  while(degval < 0.0)
...
bool PearlPID::buildReport()
{
  ...
  string sHeading      = doubleToString(m_desired_heading, 2);
  string sSpeed        = doubleToString(m_desired_speed, 2);
  string sRudder       = doubleToString(m_current_rudder, 2);
  string sThrust       = doubleToString(m_current_thrust, 2);
  string sControl      = boolToString(m_has_control);
  string sStation      = boolToString(m_station_keep);
  string sUseSolar     = boolToString(m_use_solar);
